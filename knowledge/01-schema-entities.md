...
These are non-negotiable rules derived from the schema and rendering code, not otherwise stated
above (schema-enforced rules already covered by the field tables and cross-entity list above are
not repeated here):
...
- **Blank nodes**: Exactly `{"type":"blank","attrs":{"questionId":"<uuid>"}}`. Inline atom nodes inside a paragraph/cell.
